...
make_fixture() {
  local dir="$1" root_url="${2:-git+https://codeberg.org/ziglang/translate-c#root-commit}"
...
.{
    .dependencies = .{
        ...
        .translate_c = .{
            .url = "$root_url",
            .hash = "translate_c-root-test-hash",
        },
    },
}
EOF
...
.{
    .dependencies = .{
        .translate_c = .{
            .url = "git+https://codeberg.org/ziglang/translate-c?ref=zig-0.16.x#transitive-commit",
...
ZON
      printf '%s\n' supermd-test-hash
      ;;
    git+https://github.com/ziglang/translate-c#root-commit)
      printf '%s\n' "${FAKE_ROOT_RESULT:-translate_c-root-test-hash}"
      ;;
    git+https://github.com/ziglang/translate-c?ref=zig-0.16.x#transitive-commit)
      printf '%s\n' translate_c-transitive-test-hash
      ;;
...
if [ ! -e "$TMP/warm-only/attempts" ] \
  && grep -qF 'WARMED_TRANSLATE_C_HASHES=translate_c-root-test-hash translate_c-transitive-test-hash' "$TMP/warm-only/github-env"; then
...
if FAKE_ROOT_RESULT=wrong-hash run_rescue "$TMP/mismatch" >/dev/null 2>&1; then
...
make_fixture "$TMP/host" 'git+https://example.invalid/ziglang/translate-c#root-commit'

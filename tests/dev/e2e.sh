#!/usr/bin/env bash
# e2e for `zigapagos e2e`: serve a RELEASE output tree with "zigbase", probe
# readiness, export ZIGAPAGOS_ORIGIN, run the child, tear down, propagate the
# exit code — driven against examples/tsx-site, which is a real site (four
# islands, five SPAs) rather than a fixture.
#
...
#
# Asserts:
#   (a) the child runs with ZIGAPAGOS_ORIGIN exported and — with NO polling of
#       its own — immediately fetches the islands page and prerendered SPA
#       shells from the RELEASE tree,
#   (b) teardown on success: origin dead + no stub process left,
#  (a2) the `.spa`-marker fallback serves non-enumerated SPA routes, and the
#       harness threads `--data-dir` through to the server,
#   (c) a failing child fails the command (exit-code propagation) and teardown
#       still happens,
#   (d) no command after `--` fails fast with usage guidance,
#   (e) locator: with no zigbase anywhere, it fails with actionable
...
# --- the tree under test --------------------------------------------------------
# `zigapagos e2e` only SERVES; it never builds. So the tree has to exist first,
# and it is built the way the project itself builds it.
echo "building examples/tsx-site (its own build.sh)..."
ZIGAPAGOS_BIN="$ZIGAPAGOS" bash "$SITE/build.sh" > "$WORK/build.log" 2>&1 \
  || { tail -40 "$WORK/build.log"; fail "examples/tsx-site build.sh failed"; }
[[ -d "$OUT" ]] || fail "release tree $OUT missing after build.sh"

# --- (e) locator: nothing anywhere -> clear instructions ------------------------
# Run BEFORE the stub goes on PATH. Skipped when the machine has a real zigbase
# (PATH) or the pinned release in the cache — the locator would find it. The
# pinned version comes from the file that owns it, not a copy: this check has to
# name the SAME cache dir the locator builds, and a stale literal here would look
# at an empty directory, decide no zigbase exists, and then run the case with one
# on the machine after all.
...
if command -v zigbase >/dev/null 2>&1 || [[ -x "$CACHE_PIN" ]]; then
  ...
  echo "running: zigapagos e2e -- true (no zigbase anywhere)..."
  if mise exec -- "$ZIGAPAGOS" e2e --site="$OUT" -- true > "$WORK/nozb.log" 2>&1; then
...
# contract is that the origin already answers when the child starts.
...
echo "running: zigapagos e2e -- bash child.sh (success case, stub zigbase on PATH)..."
ORIGIN_FILE="$ORIGIN_FILE" mise exec -- "$ZIGAPAGOS" e2e --site="$OUT" -- bash "$CHILD" \
  > "$WORK/ok.log" 2>&1 || { cat "$WORK/ok.log"; fail "success-case e2e failed"; }
...
# --- (a2) .spa-marker fallback + data-dir threading ------------------------------
# The tsx-site dogfood builds with deploy_target=nginx, so re-run the emitter
# with --target zigbase against the built tree (exactly like test/spa.sh) to
# plant the `.spa` markers the stock server reads, then drive `zigapagos e2e`
# with an observable --data-dir.
...
echo "running: zigapagos e2e --data-dir=... (.spa fallback)..."
mise exec -- "$ZIGAPAGOS" e2e --site="$OUT" --data-dir="$DATA" -- bash "$DIRECT_CHILD" \
  > "$WORK/direct.log" 2>&1 || { cat "$WORK/direct.log"; fail "e2e with --data-dir failed"; }
...
echo "PASS: .spa-marker fallback served /app/nope; data dir threaded"

# --- (c) failing child: exit code propagates, teardown still happens -------------
ORIGIN_FILE2="$WORK/origin2"
echo "running: zigapagos e2e -- bash -c 'record origin; exit 7' (failure case)..."
if mise exec -- "$ZIGAPAGOS" e2e --site="$OUT" -- \
     bash -c "echo \"\$ZIGAPAGOS_ORIGIN\" > '$ORIGIN_FILE2'; exit 7" > "$WORK/fail.log" 2>&1; then
  cat "$WORK/fail.log"; fail "a child exiting 7 did not fail the e2e command"
fi
...
echo "PASS: failing child fails the command, teardown still happens"

# --- (d) no command: fail fast with usage guidance -------------------------------
echo "running: zigapagos e2e (no command)..."
if mise exec -- "$ZIGAPAGOS" e2e --site="$OUT" > "$WORK/nocmd.log" 2>&1; then
  cat "$WORK/nocmd.log"; fail "e2e without a command should fail"
fi
...
echo "PASS: e2e harness (serve + readiness + ZIGAPAGOS_ORIGIN + teardown + exit codes + locator)"

...
| `zigapagos inspect-output` | no | no | Inventories raw output bytes and page references; optional byte budgets. |
...
`bun = "1.4.2"` in `mise.toml`, and that pin is what the npm package's `bun`

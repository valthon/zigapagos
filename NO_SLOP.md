...
| 4 | **Arena-scoped** | `fn f(arena: RenderArena, …) !T` | Interlinked graph reclaimed by dropping the arena. Signature-enforced by the marker type `RenderArena` (`src/islands/render_arena.zig`, modelled on zigbase's `RequestArena`): it is deliberately NOT an `Allocator`, and its production constructor `from(*std.heap.ArenaAllocator)` takes the concrete arena, so **a GPA cannot flow in by accident** and the compiler — not a parameter *name* — checks it. Zig has no private fields, so a deliberate `RenderArena{ .a = gpa }` literal still compiles: what the type stops is accidental misuse, and any bypass (including stashing `.a` past the end of the pass) is greppable rather than impossible. `.a` is the sanctioned bridge to contract-1 helpers. Still needs a written justification. |

- **Flag:** a function that allocates scratch and returns without freeing it, relying on the caller having passed an arena. That is contract 4 without the type — the defect this section exists to stop. Same flag for a **borrowed-or-owned return** (`if (nothing_to_do) return input;` else allocate): the caller cannot free it without knowing which branch ran, so it is only sound where nobody frees. Make it always-owned (contract 1) or take a `RenderArena`.
- **Contract 4 must be earned.** All three must hold: (1) the result is a graph of interlinked allocations, not a single buffer; (2) freeing them individually would be pointer-chasing for no benefit; (3) the lifetime is genuinely pass-scoped — here a page render or SPA prerender pass, the boundary `RenderArena` is named for. "It is currently written that way" and "adding `defer`s is tedious" are NOT justifications.
- **Reviewer check (mechanical):** *Which contract is this function? Does its test use raw `std.testing.allocator` (leak detection ON)? If it takes a `RenderArena`, where is the written justification — and does it meet all three conditions?* A plain `std.mem.Allocator` parameter merely *named* `arena` is now a smell in itself: the type exists, so either the function is contract 1/2/3 (name it `alloc`) or it should say `RenderArena`.
- **Wrapping the testing allocator in an arena disables Zig's leak detector** for that test — the arena frees everything at `deinit`, so a missing `defer free` in the code under test can never be observed. Legitimate only for contract 4.
  **`RenderArena.forTest` is the way OFF this list.** It installs a non-arena allocator behind an arena-scoped signature, so a test satisfies the type while keeping the leak detector on — and that is a *harsher* execution, since arena-scoped code must be correct under any allocator anyway (the contract-1 helpers it reaches through `.a` are). Reach for it first; a row in the allowlist is the fallback for code that genuinely cannot run on a GPA (a `*Leaky` parse graph with no `deinit`). Swapping the arena for another non-detecting allocator (a stack `FixedBufferAllocator`, say) is **not** a fix — it hides the site from the scanner and restores no detection.
  **Three spellings, not two.** `ArenaAllocator.init(std.testing.allocator)` and `ArenaAllocator.init(testing.allocator)` are the obvious ones; the spelling this repo uses *most* is `ArenaAllocator.init(gpa)` with a preceding `const gpa = std.testing.allocator;`. Most sites under `src/` are alias-spelled, so a grep for the two literal forms misses the majority of them. Any reviewer check that greps must resolve the alias. (Deliberately no totals here: they move with every commit, and a hard-coded one would be exactly the stale-doc defect §6.2 flags. The rows in `scripts/allocator-allowlist.txt` *are* the inventory; `--list` prints the live one.)
  **The gate:** every file that has any such site is listed — with a count and a one-line written justification — in `scripts/allocator-allowlist.txt`. `scripts/check-allocator-contracts.sh` resolves aliases scoped by brace depth (so a function *parameter* named `gpa` is not a false positive) and fails when a file grows a new site or an unlisted file acquires one. Run it with `--list` for the current inventory; it runs in CI.
  The remaining rows are not a pile of broken tests. Almost all of them exercise an API that takes a `RenderArena` and parses with a `*Leaky` entry point (`ziggy.parseLeaky` — the only dynamic-value parse ziggy has — or `std.json.parseFromSliceLeaky`) into a graph with no `deinit`, which the raw testing allocator therefore could not free; the two exceptions carry their own reason on the row (`render_arena.zig`, where the arena is the thing under test, and `cli/migrate_detect.zig`, a genuine contract 4 that cannot *reach* the marker type: it is its own test-module root and Zig 0.16 forbids an import escaping a module's directory). Every row says which. There are no `GRANDFATHERED` rows left; do not add one.
...
> **This repo:** the high-cardinality structures are `src/Variant.zig`'s `pages` and `sections` (`ArrayListUnmanaged(Page)` / `ArrayListUnmanaged(Section)` — one entry per content file, walked by every scan/parse/render pass), the `src/StringTable.zig` / `src/PathTable.zig` entries they index into, and the island-usage records in `src/islands/manifest.zig` (one `Use` per mounted island per page). That is where §4 bites — not on one-shot config structs or argv parsing.
>
> Two things to check before filing a §4 finding here, so you don't report work that is already done or propose a change nobody has ever made:
> - **Index-instead-of-pointer is already applied.** `StringTable.String` and `PathTable.Path` are `enum(u32)` handles into a shared byte array, and `Variant.Section` stores `u32` indices into `pages`. Re-reporting it is noise.
> - **`std.MultiArrayList` appears zero times in first-party code.** An SoA suggestion is therefore a genuinely new change, not a consistency fix — bring the measurement §4 demands, not an assertion.

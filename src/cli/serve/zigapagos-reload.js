function log() {
  console.log("[Zigapagos Reloader]", ...arguments);
}

function zigapagosConnect() {
  let socket = new WebSocket("ws://" + window.location.host + "/__zigapagos/ws");
  ...
  socket.addEventListener("message", (event) => {
    ...
    if (msg.command == "reload_all") {
      // Let @z/runtime persist in-memory component state (form inputs, wizard
      // step) so it survives the full-page dev reload. Dev-only: this script is
      // injected only by `zigapagos serve`, so the event never fires in production.
    ...
    if (msg.command == "reload") {
      ...
      if (msg.path.endsWith(".html")) {
        ...
        for (let i = 0; i < links.length; i++) {
          const link = links[i];
          if (link._zigapagos_temp) continue;
      ...
      const id = "__zigapagos_build_box";
      if(msg.err != "") {
        ...
        if (box == null) {
          ...
          box.innerHTML = "<h1 style=\"color: red\">ZIGAPAGOS BUILD ERROR</h1>" ;
        }
  ...
  socket.addEventListener("close", (event) => {
    log("close", event);
    setTimeout(zigapagosConnect, 3000);
  });

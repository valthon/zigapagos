...
fn buildBindings(ctx: *Ctx) Allocator.Error!Bindings {
    ...
    for (order) |i| {
        ...
        // Layouts and their partials carry answerable controls too. Use the
        // same first-route ownership and visited set as ordinary views.
        if (d.route_templates[i].layout) |layout|
            try bindTemplate(ctx, &acc, i, layout, &seen);
    }
...
fn ensureLayout(ctx: *Ctx, acc: *Acc, cache: *LayoutCache, layout_path: []const u8) WriteError!?usize {
    ...
    const output = convert.convert(gpa, .{
        ...
        .layout_stem = null,
        // Layout controls and layout-rendered partials share the binding pass.
...
// Zig embeds bytes, not checked TypeScript. runtime/test/rails-stimulus.test.ts
// imports this template into runtime's tsc graph; preserve that check if moved.
const stimulus_runtime = @embedFile("stimulus.ts");
...
test "write: forms reached through a view or layout bind under their own name" {
    for ([_]bool{ false, true }) |layout_only| {
        const gpa = testing.allocator;
        ...
        const frags = [_]fragments.Template{
            tTemplate(bound_view, if (layout_only) &.{tText("<h1>Page</h1>", 1)} else &new_nodes),
            tTemplate("app/views/layouts/application.html.erb", &.{tNode(.render_partial, 1, 1, "posts/form")}),
        ...
        var rts = [_]rails.RouteTemplates{
            .{ .templates = &.{}, .layout = null },
            .{ .templates = &new_tpls, .layout = if (layout_only) "app/views/layouts/application.html.erb" else null },
        };
        ...
        const view = try readTarget(gpa, &tmp, if (layout_only) "layouts/templates/application.shtml" else "layouts/posts/new.shtml");
        defer gpa.free(view);
...
test "Stage 4 shared interactivity emitters have their runtime contracts" {
    ...
    try testing.expect(std.mem.indexOf(u8, stimulus_runtime, "owner.querySelectorAll<HTMLElement>(\"[data-action]\")") != null);
    try testing.expect(std.mem.indexOf(u8, stimulus_runtime, "matchesKey(e, filter)") != null);
    try testing.expect(std.mem.indexOf(u8, stimulus_runtime, "removeEventListener(event, wrapped, options)") != null);
}
